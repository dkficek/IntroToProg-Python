...
#Dan Ficek,2.10.2022, Added code to complete assignment 5
# ------------------------------------------------------------------------ #
...
strPri=""  # A user-input priority to add to the To-Do List
strQuitDec= ""  #user Y or N to quit program
...
while (True):
    ...
    # Step 4 - Add a new item to the list/Table
        ...
        print("Great! Let's add an item to your To-Do list.")
    ...
    # Step 5 - Remove a new item from the list/Table
        ...
        strTask=input("\nEnter the task you would like to remove:")   #ask user for removal choice
        for row in lstTable:
            if row['task'].lower()==strTask.lower():       #compare each task to what the user entered.
                ...
                print("Task: "+strTask+" removed from list!\n")
                                                      #  if it matches what the user wanted removed, else do nothing
    ...
    # Step 6 - Save tasks to the ToDoList.txt file
        ...
        for row in lstTable:                                        #unpack the table into tasks and priorities
            objFile.write(row['task']+","+row['priority']+"\n")

        objFile.close()
        print("\nData saved to file!\n")              #notify the user that their data is saved
        continue
    # Step 7 - Exit program after verifying user wants to do so
    elif (strChoice.strip() == '5'):
        strQuitDec=input("\nAre you sure you want to quit?  Enter Y or N:  ")
        if strQuitDec.lower() == "y":
